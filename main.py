...
<script>
let token=localStorage.getItem('token')||'';let providers=[];let movements=[];let editingId=null;
...
function showForm(){editingId=null;el('formTitle').textContent='Nuevo registro';el('saveBtn').textContent='Guardar';clearForm();el('formCard').classList.remove('hidden');el('fecha').value=new Date().toISOString().slice(0,10)}
function hideForm(){el('formCard').classList.add('hidden')}
function cancelEdit(){editingId=null;hideForm();clearForm()}
...
el('proveedor').addEventListener('input',()=>{let nombre=el('proveedor').value.trim().toLowerCase();let p=providers.find(x=>x.name.toLowerCase()===nombre);if(p){el('rut').value=p.rut;el('rut').readOnly=true;el('rut').style.background='#eee'}else{el('rut').readOnly=false;el('rut').style.background='#fff'}});
